import { AudioFeatures, Recommendations, SearchContent, Track } from 'spotify-types';
...
export const getSongData = async (songId: string): Promise<Track> => {
  const res = await fetch('https://api.spotify.com/v1/tracks/' + songId, {
    headers: {
      Authorization: 'Bearer ' + (window as any).creds.access_token,
    },
  });
  if (res.status !== 200)
    throw new Error(
      res.status +
        ' Failed request... track - ' +
        songId +
        ' ' +
        (await res.text())
    );
...
export const getLyrics = async (songId: string) => {
  const res = await fetch(
    'https://corsproxy.io/?https://lyrix.vercel.app/getLyrics/' + songId
  );
  if (res.status !== 200) {
    if (res.status === 500) return null;
    else throw new Error(res.status + ' Failed request... lyrics - ' + songId);
  }
  const data = await res.json();
  return data;
};
...
export const getRecommendations = async (
  ...
  if (res.status !== 200)
    throw new Error(
      res.status + ' Failed request... recommendations - ' + songId
    );
  const data = await res.json();
  return data;
};
...
export const getSpotifyResults = async (q: string): Promise<SearchContent> => {
  ...
  if (res.status !== 200)
    throw new Error(
      res.status + ' Failed request... search - ' + q
    );
  const data = await res.json();
  return data;
}